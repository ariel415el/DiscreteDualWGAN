...
def OTS(outputs, psi, opt_psi, data, loss_func, args, it, patch_mode):
    ...
    for ots_iter in range(args.max_ots_iters):
        ...
        output_idx = torch.randperm(len(outputs))[:args.batch_size]

        phi, data_idx = loss_func.score(outputs[output_idx], data, psi)
...
def FIT(outputs, opt, psi, data, loss_func, patch_mode):
    opt.zero_grad()
    losses = []
    total_loss = 0
    for _ in range(args.memory_size):
        if patch_mode:
        ...
        loss = loss_func.loss(data[data_idx], outputs_[output_idx])
...
def train(args, patch_mode=False):
    ...
    for it in range(args.n_iters):
        ...
        g_loss = FIT(outputs, opt_outputs, psi.detach(), data, loss_func, patch_mode)
